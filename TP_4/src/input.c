...
int inputString(char* input, char message[], char eMessage[], int minLimit, int maxLimit, int numberDisabled)
{
    int returnValue = -1;
    int counter = 0;
    int sizeScan = 0;
    ...
    if(input != NULL && message != NULL && eMessage != NULL && maxLimit >= minLimit && maxLimit < MAXCHAR && minLimit > 0)
    {
       ...
       // } while(sizeScan < minLimit || sizeScan > maxLimit || onlyLetters(auxMessage)==0 );
             } while(sizeScan < minLimit || sizeScan > maxLimit || onlyLetters(auxMessage)==numberDisabled );
...
int inputStringAndNumber(char* input, char message[], char eMessage[], int minLimit, int maxLimit)
{
    int returnValue = -1;
    int counter = 0;
    int sizeScan = 0;
...
/*void inputStringToInt2(char inputStr[], int* ouputInt)
{
    int retorno = 1;
    //int valor;

	*ouputInt =  atoi(inputStr);
	retorno = 0;
...
int inputStringToFloat(char inputStr[], float* ouputFloat)
{
    int retorno = 0;
    int valor;
...
char checkContinue()
{
    ...
    inputString(&continuar, "Confirme para continuar [S]i [N]o : ", "Confirme para continuar [S]i [N]o : ", 1, 1, 0);
